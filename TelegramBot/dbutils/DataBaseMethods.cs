...
public static class DataBaseMethods
{
	...
	public static IEnumerable<IList<ReminderSchedule>> GetSchedules()
	{
		using (ApplicationContext db = new ApplicationContext())
		{
			var offsetNum = 0;
			const int LIMIT_NUM = 100;
			IList<ReminderSchedule> schedules;
			do
			{
				schedules = db.Schedules.FromSql($"SELECT * FROM public.\"Schedules\" LIMIT {LIMIT_NUM} OFFSET {offsetNum}").ToList();
